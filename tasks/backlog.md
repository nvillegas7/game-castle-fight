...
| CASTLE-CLIFF | **Castle sits on an integrated cliff base** (user-flagged 2026-07-11, ref `design/references/v1.png` red castle). Currently each castle sits on a single full-width stone-face row (`compose_arena.py:165-174` `tile_of(GRASS,6,4)` at WALL_Y 103/911, x140-580; ported in `game_arena.gd:_add_fortress_dressing:1429`). Want per the reference: a **taller stone cliff FACE directly under the castle front (south)** + a **grass-line rim divider at the back (north)** — an INTEGRATED raised mound, castle-width. Both castles face SOUTH (no flip; stone tile is never flipped). **Compose-first** (`tools/compose_arena.py`→approve `arena_target.png`→port to `_add_fortress_dressing`). Constraints found: a detached 2nd stone row one tile south reads WRONG (grass gap, floating strip) — the base must be integrated/contiguous with the castle sprite's own stone foot; the BLUE castle sits at the platform bottom edge (PLAT_Y1=968, stone bottom 975) so a south-facing face below it has little room — may need the blue castle nudged up or a shorter/edge-blended face. Needs a few compose iterations. | A2 | arena | side-by-side vs v1.png red castle; arena parity detectors green |
